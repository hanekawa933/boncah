...
const GraphProduksiPupukYearly = () => {
  const date = new Date().getFullYear();
  const [year, setYear] = useState(date);

  const [produksiPupukYearly, setProduksiPupukYearly] = useState([]);
  ...
  const fetchYearlyProduksiPupuk = async (query) => {
    try {
      const res = await instance.get(query);
      setProduksiPupukYearly(res.data);
    } catch (error) {
      alert("ERROR FROM GRAPH PRODUKSI PUPUK YEARLY");
    }
  };
  useEffect(() => {
    month.length >= 1
      ? fetchYearlyProduksiPupuk(
          `/pupuk/produksi/yearly?year=${year}&month=${monthToIndex.join()}`
        )
      : fetchYearlyProduksiPupuk(`/pupuk/produksi/yearly?year=${year}`);
  ...
  for (let i = 0; i < keyObj.length; i++) {
    // eslint-disable-next-line no-unused-expressions
    if (produksiPupukYearly && produksiPupukYearly.result) {
      if (produksiPupukYearly.result[0][keyObj[i]] !== undefined) {
        arr.push(produksiPupukYearly.result[0][keyObj[i]]);
      }
    }
  }
  ...
  const availableYear =
    produksiPupukYearly && produksiPupukYearly.listOfYear
      ? produksiPupukYearly.listOfYear.map((res, index) => {
          return (
            <Button
              key={index}
              value={res.tahun}
...
export default GraphProduksiPupukYearly;
