...
const GraphPenjualanSusuMonthly = () => {
  const date = new Date().getFullYear();
  const currMonth = new Date().getUTCMonth() + 1;
  const [penjualanSusuMonthly, setPenjualanSusuMonthly] = useState([]);
  ...
  const labelForTime =
    penjualanSusuMonthly && penjualanSusuMonthly.listOfJID
      ? penjualanSusuMonthly.listOfJID.map((res) => {
          return {
            key: res.id,
            id: res.id.toString(),
  ...
  const fetchMonthlyPenjualanSusu = async (query) => {
    try {
      const res = await instance.get(query);
      setPenjualanSusuMonthly(res.data);
    } catch (error) {
  ...
  useEffect(() => {
    month !== null && variant !== null
      ? fetchMonthlyPenjualanSusu(
          `/susu/penjualan/monthly?year=${year}&month=${month}&jenis_susu_id=${variant}`
        )
      : fetchMonthlyPenjualanSusu(
          `/susu/penjualan/monthly?year=${year}&jenis_susu_id=${variant}`
        );
  ...
  for (let i = 0; i < monthLength; i++) {
    // eslint-disable-next-line no-unused-expressions
    if (penjualanSusuMonthly && penjualanSusuMonthly.result) {
      if (penjualanSusuMonthly.result[0][i + 1] !== undefined) {
        arr.push(penjualanSusuMonthly.result[0][i + 1]);
      }
    }
  }
  ...
  const availableYear =
    penjualanSusuMonthly && penjualanSusuMonthly.listOfYear
      ? penjualanSusuMonthly.listOfYear.map((res, index) => {
          return (
            <Button
              key={index}
              value={res.tahun}
...
export default GraphPenjualanSusuMonthly;
