...
const GraphPenjualanSusuYearly = () => {
  const date = new Date().getFullYear();
  const [year, setYear] = useState(date);
  const [penjualanSusuYearly, setPenjualanSusuYearly] = useState([]);
  ...
  const labelForTime =
    penjualanSusuYearly && penjualanSusuYearly.listOfJID
      ? penjualanSusuYearly.listOfJID.map((res, index) => {
          return {
            key: index,
            id: res.id.toString(),
  ...
  const fetchYearlyPenjualanSusu = async (query) => {
    try {
      const res = await instance.get(query);
      setPenjualanSusuYearly(res.data);
    } catch (error) {
  ...
  useEffect(() => {
    month.length >= 1 && variant !== null
      ? fetchYearlyPenjualanSusu(
          `/susu/penjualan/yearly?year=${year}&month=${monthToIndex.join()}&jenis_susu_id=${variant}`
        )
      : fetchYearlyPenjualanSusu(
          `/susu/penjualan/yearly?year=${year}&jenis_susu_id=${variant}`
        );
  ...
  for (let i = 0; i < keyObj.length; i++) {
    // eslint-disable-next-line no-unused-expressions
    if (penjualanSusuYearly && penjualanSusuYearly.result) {
      if (penjualanSusuYearly.result[0][keyObj[i]] !== undefined) {
        arr.push(penjualanSusuYearly.result[0][keyObj[i]]);
      }
    }
  }
  ...
  const availableYear =
    penjualanSusuYearly && penjualanSusuYearly.listOfYear
      ? penjualanSusuYearly.listOfYear.map((res, index) => {
          return (
            <Button
              key={index}
              value={res.tahun}
...
export default GraphPenjualanSusuYearly;
